...
function setupEventListeners() {
  ...
  document.querySelectorAll('.emoji-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      document.getElementById('imageUrl').value = this.dataset.emoji;
    });
  });
...
function makeRecipeCard(r) {
  ...
  const isEmoji = r.image_url && !r.image_url.match(/^(http|\/|data:)/i) && r.image_url.trim().length <= 10;
  const imageHtml = isEmoji 
    ? `<div class="recipe-image-emoji">${r.image_url}</div>`
    : `<img src="${r.image_url || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400'}" alt="${r.name}" loading="lazy">`;
...
// ==========================================
// 🍳 COMPREHENSIVE RECIPE DETAIL MODAL
// ==========================================

function viewRecipeDetails(recipeId) {
  const recipe = SAMPLE_DATA.recipes.find(r => r.id === recipeId);
  if (!recipe) {
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 800px;">
      <span id="closeDetailModal" style="position: absolute; top: 20px; right: 25px; font-size: 2rem; cursor: pointer; color: var(--text-secondary);">&times;</span>
      
      <div style="margin-bottom: 2rem;">
        <h2 style="margin: 0 0 0.5rem 0; font-size: 2rem; color: var(--text-primary);">${recipe.name}</h2>
        <p style="margin: 0; color: var(--text-secondary); font-size: 1.1rem;">
          <strong>${recipe.chef}</strong> ${recipe.chefAvatar || '👨‍🍳'}
        </p>
      </div>
      
      <div style="width: 100%; height: 350px; border-radius: 20px; overflow: hidden; margin-bottom: 2rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); display: flex; align-items: center; justify-content: center;">
        <div style="font-size: 10rem;">${recipe.image}</div>
      </div>
      
      <div style="display: flex; gap: 1rem; margin-bottom: 2rem; flex-wrap: wrap;">
        <span style="padding: 0.6rem 1.2rem; background: var(--bg-secondary); border-radius: 12px; font-size: 0.95rem;">
          <strong>⏱️ ${recipe.cookTime} min</strong>
        </span>
        <span style="padding: 0.6rem 1.2rem; background: var(--bg-secondary); border-radius: 12px; font-size: 0.95rem;">
        ...
        <span style="padding: 0.6rem 1.2rem; background: ${getDifficultyColor(recipe.difficulty)}; border-radius: 12px; font-size: 0.95rem; color: white;">
      ...
      <div style="display: flex; gap: 1rem; margin-bottom: 2rem; flex-wrap: wrap; align-items: center;">
        <span style="font-size: 1.2rem;">❤️ <strong>${recipe.likes}</strong> likes</span>
        <span style="font-size: 1.2rem;">⭐ <strong>${recipe.avgRating.toFixed(1)}</strong> rating</span>
        <span style="font-size: 1.2rem;">👁️ <strong>${recipe.views}</strong> views</span>
        <span style="font-size: 1.2rem;">💬 <strong>${recipe.comments}</strong> comments</span>
      </div>
      
      <div style="display: flex; gap: 1rem; margin-bottom: 2rem; flex-wrap: wrap;">
        <button onclick="handleLikeRecipe(${recipe.id})" style="flex: 1; min-width: 140px; padding: 0.8rem 1.5rem; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; border: none; border-radius: 12px; font-size: 1rem; cursor: pointer; transition: transform 0.2s;">
          <i class="fas fa-heart"></i> Like
        </button>
        <button onclick="handleSaveRecipe(${recipe.id})" style="flex: 1; min-width: 140px; padding: 0.8rem 1.5rem; background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; border: none; border-radius: 12px; font-size: 1rem; cursor: pointer; transition: transform 0.2s;">
          <i class="fas fa-bookmark"></i> Save
        </button>
        <button onclick="handleShareRecipe(${recipe.id})" style="flex: 1; min-width: 140px; padding: 0.8rem 1.5rem; background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%); color: white; border: none; border-radius: 12px; font-size: 1rem; cursor: pointer; transition: transform 0.2s;">
          <i class="fas fa-share"></i> Share
        </button>
      </div>
      
      <div style="margin-bottom: 2.5rem; padding: 2rem; background: var(--bg-secondary); border-radius: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        <h3 style="margin: 0 0 1.5rem 0; font-size: 2rem; color: var(--text-primary); font-weight: 700;">🥘 Ingredients</h3>
        <ul style="margin: 0; padding-left: 2rem; line-height: 2.2; font-size: 1.15rem;">
          ${recipe.ingredients.split(',').map(ing => `<li style="margin-bottom: 0.8rem; padding-left: 0.5rem;"><strong>${ing.trim()}</strong></li>`).join('')}
        </ul>
      </div>
      
      <div style="margin-bottom: 2.5rem; padding: 2rem; background: var(--bg-secondary); border-radius: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        <h3 style="margin: 0 0 2rem 0; font-size: 2.2rem; color: var(--text-primary); font-weight: 700;">👨‍🍳 How to Make This Recipe</h3>
        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
          ${(() => {
            const steps = recipe.instructions.split(',').filter(step => step.trim());
            return steps.map((step, index) => {
              const stepNumber = index + 1;
              const stepText = step.trim().replace(/^(step\s*\d+:?\s*|\d+\.\s*|\d+\)\s*)/i, '');
              return `
              <div style="display: flex; gap: 1.5rem; align-items: flex-start; padding: 1.5rem; background: var(--bg-primary); border-radius: 15px; border-left: 4px solid #667eea; transition: transform 0.2s; cursor: pointer;" onmouseover="this.style.transform='translateX(5px)'" onmouseout="this.style.transform='translateX(0)'">
                <div style="flex-shrink: 0; width: 50px; height: 50px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; font-weight: 700; box-shadow: 0 4px 8px rgba(102, 126, 234, 0.3);">
                  ${stepNumber}
                </div>
                <div style="flex: 1;">
                  <div style="font-size: 0.85rem; color: #667eea; font-weight: 600; margin-bottom: 0.5rem; text-transform: uppercase; letter-spacing: 0.5px;">Step ${stepNumber}</div>
                  <div style="font-size: 1.15rem; line-height: 1.8; color: var(--text-primary);">${stepText}</div>
                </div>
              </div>
            `}).join('');
          })()}
        </div>
        <div style="margin-top: 2rem; padding: 1rem; background: linear-gradient(135deg, rgba(102, 126, 234, 0.1) 0%, rgba(118, 75, 162, 0.1) 100%); border-radius: 12px; text-align: center;">
          <p style="margin: 0; font-size: 0.95rem; color: var(--text-secondary);">
            <strong>${recipe.instructions.split(',').filter(s => s.trim()).length} steps</strong> • Estimated time: <strong>${recipe.cookTime} minutes</strong>
          </p>
        </div>
      </div>
      
      <div style="margin-bottom: 2rem;">
        <h3 style="margin: 0 0 1rem 0; font-size: 1.3rem;">🏷️ Tags</h3>
        <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
          ${recipe.tags.split(',').map(tag => `
            <span style="padding: 0.5rem 1rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 20px; font-size: 0.9rem;">
              ${tag.trim()}
            </span>
          `).join('')}
        </div>
      ...
      <div style="margin-bottom: 2rem; padding: 1.5rem; background: var(--bg-secondary); border-radius: 15px;">
        <h3 style="margin: 0 0 1rem 0; font-size: 1.5rem;">⭐ Rate this Recipe</h3>
        <div id="ratingStars" style="font-size: 2.5rem; cursor: pointer; margin-bottom: 0.5rem;">
          <i class="far fa-star" data-rating="1" onclick="handleRateRecipe(${recipe.id}, 1)" style="color: #FFD700; transition: transform 0.2s;"></i>
          <i class="far fa-star" data-rating="2" onclick="handleRateRecipe(${recipe.id}, 2)" style="color: #FFD700; transition: transform 0.2s;"></i>
          <i class="far fa-star" data-rating="3" onclick="handleRateRecipe(${recipe.id}, 3)" style="color: #FFD700; transition: transform 0.2s;"></i>
          <i class="far fa-star" data-rating="4" onclick="handleRateRecipe(${recipe.id}, 4)" style="color: #FFD700; transition: transform 0.2s;"></i>
          <i class="far fa-star" data-rating="5" onclick="handleRateRecipe(${recipe.id}, 5)" style="color: #FFD700; transition: transform 0.2s;"></i>
        </div>
        <p style="margin: 0; color: var(--text-secondary); font-size: 0.9rem;">Current Rating: ${recipe.avgRating.toFixed(1)} ⭐ (${recipe.comments} ratings)</p>
      </div>
      
      <div style="margin-bottom: 2rem; padding: 1.5rem; background: var(--bg-secondary); border-radius: 15px;">
        <h3 style="margin: 0 0 1rem 0; font-size: 1.5rem;">💬 Comments (${recipe.comments})</h3>
        <div id="commentsList" style="max-height: 300px; overflow-y: auto; margin-bottom: 1rem;">
          <p style="color: var(--text-secondary); text-align: center; padding: 2rem;">
            Comments coming soon! 🎉
          </p>
        </div>
        <textarea id="commentInput" placeholder="Share your thoughts about this recipe..." rows="3" style="width: 100%; padding: 1rem; border: 2px solid var(--bg-secondary); border-radius: 12px; font-family: 'Poppins', sans-serif; font-size: 0.95rem; margin-bottom: 0.5rem; background: var(--bg-primary); color: var(--text-primary);"></textarea>
        <button onclick="handleAddComment(${recipe.id})" style="width: 100%; padding: 0.8rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 12px; font-size: 1rem; cursor: pointer; transition: transform 0.2s;">
...
function handleLikeRecipe(recipeId) {
  if (!authToken) {
    alert("Login to like recipes! 🍰");
    return;
  }
  alert("Recipe liked! ❤️");
}
...
function handleShareRecipe(recipeId) {
...
function handleRateRecipe(recipeId, rating) {
  if (!authToken) {
    alert("Login to rate recipes! 🍰");
    return;
  }
  alert(`You rated this recipe ${rating} star${rating > 1 ? 's' : ''}! ⭐`);
}

function handleAddComment(recipeId) {
  if (!authToken) {
  ...
  if (!text) {
    alert("Please write a comment first! ✍️");
    return;
  }
...
async function openRecipeDetail(id) {
  ...
  // Track view
  if (authToken) {
    await fetch(`/api/recipes/${id}/view`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${authToken}` }
    }).catch(() => {});
  }
  ...
  const isEmoji = recipe.image_url && !recipe.image_url.match(/^(http|\/|data:)/i) && recipe.image_url.trim().length <= 10;
  if (isEmoji) {
    detailImageEl.style.display = 'none';
    const emojiDiv = detailImageEl.parentElement.querySelector('.detail-emoji') || document.createElement('div');
    emojiDiv.className = 'detail-emoji';
    emojiDiv.style.cssText = 'font-size: 8rem; text-align: center; padding: 2rem;';
    emojiDiv.textContent = recipe.image_url;
    if (!detailImageEl.parentElement.querySelector('.detail-emoji')) {
      detailImageEl.parentElement.insertBefore(emojiDiv, detailImageEl);
    }
  } else {
    detailImageEl.style.display = 'block';
    const emojiDiv = detailImageEl.parentElement.querySelector('.detail-emoji');
    if (emojiDiv) emojiDiv.remove();
    detailImageEl.src = recipe.image_url || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=600';
  }
...
async function toggleLike(recipeId) {
  try {
    ...
    if (likeCountEl) likeCountEl.innerText = data.likesCount;
    
    const recipe = allRecipes.find(r => r.id == recipeId);
    if (recipe) {
      recipe.likes = data.likesCount;
      
      document.querySelectorAll(`[data-id="${recipeId}"]`).forEach(card => {
        const likeSpan = card.querySelector('.fa-heart')?.parentElement;
        if (likeSpan) likeSpan.innerHTML = `<i class="fas fa-heart"></i> ${data.likesCount}`;
      });
    }
  } catch (err) {
    console.error(err);
...
async function rateRecipe(recipeId, rating) {
  try {
    ...
    if (avgRatingEl) avgRatingEl.innerText = data.avgRating.toFixed(1);
    if (ratingsCountEl) ratingsCountEl.innerText = data.ratingsCount;
    
    const recipe = allRecipes.find(r => r.id == recipeId);
    if (recipe) {
      recipe.avgRating = data.avgRating;
      
      document.querySelectorAll(`[data-id="${recipeId}"]`).forEach(card => {
        const ratingSpan = card.querySelector('.fa-star')?.parentElement;
        if (ratingSpan) ratingSpan.innerHTML = `<i class="fas fa-star"></i> ${data.avgRating.toFixed(1)}`;
      });
    }
  } catch (err) {
    console.error(err);
...
function clearRecipeForm() {
  ...
  document.getElementById("imageUrl").value = '🍽️';
...
async function postRecipe() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const currentView = document.getElementById("myRecipesView");
...
// ==========================================
// 🔍 ENHANCED SEARCH WITH SAMPLE DATA
// ==========================================
...
function setupSearchFunctionality() {
  ...
  searchInput.addEventListener('input', () => {
    ...
    searchTimeout = setTimeout(performEnhancedSearch, 300);
  });
  ...
  searchInput.addEventListener('keypress', (e) => {
    if (e.key === 'Enter') performEnhancedSearch();
  });
}
...
function performEnhancedSearch() {
  ...
  if (!query || !SAMPLE_DATA) {
    const existingHeader = document.querySelector('[data-search-header]');
...
function displaySearchResults(recipes, query) {
  ...
  const existingHeader = container.parentElement.querySelector('[data-search-header]');
  if (existingHeader) existingHeader.remove();
  
  const resultsHeader = document.createElement('div');
  resultsHeader.setAttribute('data-search-header', 'true');
  resultsHeader.style.cssText = 'margin-bottom: 1.5rem; padding: 1rem; background: var(--bg-secondary); border-radius: 15px;';
  resultsHeader.innerHTML = `
    <h3 style="margin: 0 0 0.5rem 0;">🔍 Search Results</h3>
    <p style="margin: 0; color: var(--text-secondary);">Found <strong>${recipes.length}</strong> recipe${recipes.length !== 1 ? 's' : ''} matching "${query}"</p>
  `;
  
  container.innerHTML = '';
  container.parentElement.insertBefore(resultsHeader, container);
...
async function fetchAndRenderTopRecipes() {
  try {
    const res = await fetch("/api/users/top-recipes", {
      headers: { "Authorization": `Bearer ${authToken}` }
    });
    const data = await res.json();
